...
Usage:
  ...
  or  set WOLVENKIT_GAME=...  then  python test-new-tools.py

The server DLL path is taken from WOLVENKIT_MCP_DLL, otherwise the Release
...
def main():
    if not os.path.exists(SERVER_DLL):
        sys.exit(f"Server DLL not found: {SERVER_DLL}\n"
                 "Build first: dotnet build src\\WolvenKitMcp -c Release")
    if not os.path.isdir(GAME):
        sys.exit(f"Cyberpunk 2077 installation not found: {GAME}\n"
                 "Pass the path as an argument or via WOLVENKIT_GAME.")
