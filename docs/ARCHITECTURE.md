...
- **`src/WkMcp`** — the MCP host. It speaks MCP JSON-RPC to the client
  (Claude) over stdio (or opt-in HTTP/Streamable), exposes **157 tools** (70 base +
  51 workflow/inspection + 36 live), **10 prompts** and **4 resources**, and links *no*
...
(`WolvenKitTools.cs` for the 70 base tools, `ModdingTools.cs` + its partials
`AssetInspectionTools.cs` / `GameplayInspectionTools.cs` and `SceneTools.cs` for the
51 high-level workflow / inspection tools, `LiveTools.cs` for the 36 live tools).
...
| `src/WkMcp/WolvenKitTools.cs` | 70 base MCP tools + helpers (`Structured`, `Err`, `Truncate`, `MatchesGlob`, `Snapshot`/`ProducedIn`, `BuildCpmodprojXml`). |
