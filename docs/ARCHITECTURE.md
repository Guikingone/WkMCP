...
(`WolvenKitTools.cs` for the 77 base tools, `ModdingTools.cs` + its partials
`AssetInspectionTools.cs` / `GameplayInspectionTools.cs` and `SceneTools.cs` for the
51 high-level workflow / inspection tools, `ProbeTools.cs` for the 1 diagnostic probe
tool, `LiveTools.cs` for the 36 live tools).
...
| `src/WkMcp/WolvenKitTools.cs` | 77 base MCP tools + helpers (`Structured`, `Err`, `Truncate`, `MatchesGlob`, `Snapshot`/`ProducedIn`, `BuildCpmodprojXml`). |
| `src/WkMcp/ModdingTools.cs` | 28 high-level workflow tools + framework knowledge base. |
| `src/WkMcp/AssetInspectionTools.cs` | 10 asset-inspection tools (materials / UI / rig / diff / Nexus) — `partial` of `ModdingTools`. |
| `src/WkMcp/GameplayInspectionTools.cs` | 2 gameplay-logic tools (`.questphase` / `.community`) — `partial` of `ModdingTools`. |
| `src/WkMcp/SceneTools.cs` | 11 scene (`.scene`) tools (inspect / graph / validate / translate / edit). |
| `src/WkMcp/LiveTools.cs` | 36 `live_*` tools (running game, via CetBridge). |
| `src/WkMcp/CetBridge.cs` | TCP/file bridge to the CETBridge Lua mod (live game). |
| `src/WkMcp/RedscriptParser.cs` | REDscript tokenizer + recursive parser (`lint_script`, `script_api_index`). |
| `src/WkMcp/ScriptApi.cs` | REDscript symbol index core (`script_api_index`). |
| `src/WkMcp/ProbeTools.cs` | 1 diagnostic probe tool (`game_probe` — liveness/logs/setup/crash/live verdict). |
| `src/WkMcp/SccDiagnostics.cs` | scc compiler output parser (`type_check_scripts`). |
| `src/WkMcp/TweakValidation.cs` | TweakXL typed-validation + preview core (`validate_tweak`, `preview_tweak`). |
