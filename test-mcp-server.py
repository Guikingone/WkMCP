#!/usr/bin/env python3
"""
Client de test pour le serveur MCP WolvenKit (transport stdio / JSON-RPC 2.0).

Lance le serveur, effectue le handshake MCP, liste les outils exposés, puis
appelle des outils de lecture (wolvenkit_status, compute_hash), un outil
d'écriture (pack_archive) et un outil de workflow (create_mod_project). Chaque
réponse est attendue avant l'envoi de la requête suivante.
...
# sortie du serveur). Sans cela, la console Windows (cp1252) fait planter print().
...
# dotnet : depuis le PATH (Windows, Linux), sinon repli Homebrew macOS.
DOTNET = (os.environ.get("WOLVENKIT_DOTNET")
          or shutil.which("dotnet")
          or "/opt/homebrew/opt/dotnet@8/libexec/dotnet")

# tempfile.gettempdir() est cross-platform : /tmp sous Unix, %TEMP% sous Windows.
...
def prepare_pack_fixture():
    """Crée un dossier source réel pour exercer l'outil d'écriture pack_archive."""
    ...
    with open(os.path.join(d, "sample.txt"), "w") as f:
        f.write("WolvenKit MCP pack test - contenu compressible. " * 200)
...
def main():
    ...
    def recv():
        ...
        try:
            ...
            print(f"  [ligne stdout non-JSON : {line.strip()[:200]}]")
            return None
    ...
    def tool_text(resp):
        ...
        if "error" in resp:
            return f"ERREUR JSON-RPC : {resp['error']}"
        result = resp.get("result", {})
        texts = [c.get("text", "") for c in result.get("content", [])
                 if c.get("type") == "text"]
        suffix = "   [isError=true]" if result.get("isError") else ""
        return ("\n".join(texts) or "(contenu vide)") + suffix
    ...
    def resource_text(resp):
        ...
        return "\n".join(c.get("text", "") for c in contents) or "(contenu vide)"
    ...
    print(f"=== initialize ===\n  serveur   : {res.get('serverInfo')}"
          f"\n  protocole : {res.get('protocolVersion', '?')}")
    ...
    print(f"\n=== tools/list — {len(tools)} outil(s) ===")
    for t in tools:
        print(f"  • {t['name']}")

    # 3) tools/call : wolvenkit_status (lecture)
    send({"jsonrpc": "2.0", "id": 3, "method": "tools/call",
          "params": {"name": "wolvenkit_status", "arguments": {}}})
    print("\n=== tools/call : wolvenkit_status ===")
    print(tool_text(recv()))

    # 4) tools/call : compute_hash (lecture)
    ...
    # 5) tools/call : pack_archive (écriture — exerce la compression Kraken native)
    ...
    print(f"  → fichiers .archive produits : {produced or 'aucun'}")

    # 6) tools/call : create_mod_project (workflow — système de fichiers, sans cp77tools)
    ...
    print(f"  → structure source/archive créée : {os.path.isdir(created)}")

    # 7) resources/list — ressources directes
    send({"jsonrpc": "2.0", "id": 7, "method": "resources/list", "params": {}})
    resources = (recv() or {}).get("result", {}).get("resources", [])
    print(f"\n=== resources/list — {len(resources)} ressource(s) ===")
    for r in resources:
    ...
    # 9) resources/read — ressource statique
    send({"jsonrpc": "2.0", "id": 9, "method": "resources/read",
          "params": {"uri": "wolvenkit://reference"}})
    print("\n=== resources/read : wolvenkit://reference (extrait) ===")
    print(resource_text(recv())[:400])

    # 10) resources/read — resource template (archive produite à l'étape 5)
    ...
    # 12) prompts/get — déroule une recette (read_game_file_workflow)
    ...
    if err:
        ...
        if timings:
            print("\n=== Latences serveur (request handler) ===")
            for t in timings:
                print("  " + t)
