...
(`cp77tools`) as **127 tools** an agent (Claude) can call — so you steer Cyberpunk
...
The server exposes **127 tools** (91 offline + 36 `live_*`), **8 prompts** and **4 resources**.
...
- *TweakDB* — `describe_tweak_record`, `read_tweak`, `write_tweak`, `validate_tweak`, `install_tweak`, `dump_records`, `generate_tweak_template`.
...
- **[docs/TOOLS.md](docs/TOOLS.md)** — exhaustive reference of the 127 tools + 8 prompts + 4 resources (parameters included).
...
```
wolvenkit-mcp/
├── src/WolvenKitMcp/         C# / .NET 8 MCP server (127 tools, 8 prompts, 4 resources)
